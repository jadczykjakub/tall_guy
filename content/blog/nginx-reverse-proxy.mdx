...
published: true
imageSrc: 'blog/nginx-reverse-proxy/placeholder.png'
shortDescription: 'How to use reverse proxy to controll flow in our multi docker application'
category: ['nginx', 'docker']
...
<Question text="Why can't I expose the application on port 80 so that internet calls go directly to my server" />
...
<Question text="But what if we have several web servers" />
...
<Question text="But what if we want the services to communicate with each other" />
